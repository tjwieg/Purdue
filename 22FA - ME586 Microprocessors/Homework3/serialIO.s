...
initcom     PROC    ; initializes serial channel 1 for asynchronous communcations
        ...
        orr R1, =USART1en ; turn on USART1EN
        str R1, [R3]
        ...
        ldr R3, =USART1_BRR
        ldr R1, #USART1baud ; as close as possible to 115,200 -- impossible to get exact at 24MHz
        str R1, [R3]
...
; preceded by either a space or a minus sign depending on whether the signed number is positive or negative
        ...
        mov R1, #1
        ldr R3, =array3
        ldr R0, #0x20   ; ascii character for " "
clrloop strb R0, [R3], #1
        ...
        ; is positive
        ldrb R1, #0x20 ; ascii character for " "
        ldr R3, =array3
        strb R1, [R3], #5
        b binloop

        ; is negative
neg1    ldrb R1, #0x2D ; ascii character for "-"
        ldr R3, =array3
        strb R1, [R3], #5
        sxth R0
        sub R0, #1
        ...
        ; divide by 10
binloop udiv R1, R0, #10    ; R1 holds quotient
        mul R2, R1, #10
